...
void main() {
    ...
    vec4 pixel2 = texture2D(u_TexSolid, uv);
    vec4 pixel3 = texture2D(u_TexGodrays, uv);
    
	vec4 clr = pixel1;
    clr = vec4(blendSoftLight(clr.rgb, pixel2.rgb, .15 * pixel2.a), clr.a);
    clr = vec4(blendOverlay(clr.rgb, pixel3.rgb, pixel3.a), clr.a);
    gl_FragColor = clr;
}