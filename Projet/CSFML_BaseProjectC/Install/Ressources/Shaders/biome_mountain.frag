...
void main() {
    ...
    vec4 pixel2 = texture2D(u_TexDust, uv);
    vec4 pixel3 = texture2D(u_TexGodrays, uv);
    vec4 pixel4 = texture2D(u_TexClouds, uv);
    
	vec4 clr = pixel1;
    clr = vec4(blendNormal(clr.rgb, pixel2.rgb, .6 * pixel2.a), clr.a);
    clr = vec4(blendSoftLight(clr.rgb, pixel3.rgb, pixel3.a), clr.a);
    clr = vec4(blendNormal(clr.rgb, pixel4.rgb, .4 * pixel4.a), clr.a);
    gl_FragColor = clr;
}